...
async function load(){if(!sb)return;const {data,error}=await sb.from("products").select("*").order("created_at",{ascending:false});if(error)return toast(error.message);products=data||[];$("#total").textContent=products.length;$("#active").textContent=products.filter(p=>p.is_active).length;$("#low").textContent=products.filter(p=>Number(p.stock)<=5).length;renderRecent();renderTable()}
function table(list){if(!list.length)return "<p class='muted'>Belum ada produk.</p>";return `<div class="table-wrap"><table class="table"><thead><tr><th></th><th>Product</th><th>Price</th><th>Stock</th><th>Status</th><th>Action</th></tr></thead><tbody>${list.map(p=>`<tr><td>${p.image_url?`<img class="thumb" src="${esc(p.image_url)}">`:"<div class='thumb'></div>"}</td><td><b>${esc(p.name)}</b><br><small>${esc(p.category)}</small></td><td>${money(p.price)}</td><td>${p.stock??0}</td><td>${p.is_active?"Active":"Hidden"}</td><td><div class="actions"><button data-edit="${p.id}">Edit</button><button class="del" data-del="${p.id}">Delete</button></div></td></tr>`).join("")}</tbody></table></div>`}
